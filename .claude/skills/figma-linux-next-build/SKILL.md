...
**Runtime guard for the un-connected case:** a JS-level check (`app.whenReady`-time or
earlier) cannot work here — real-device reproduction showed Chromium's native sandbox
bootstrap (`sandbox/linux/services/credentials.cc`, `userns_create`) aborts with a
"Permission denied (13)" crash and a core dump *before* Node/V8 ever executes our entry
script, so any check placed in `src/main/index.ts` is dead code that never gets a chance
to run. The only place that reliably runs first is the snap's own launcher script
(`command.sh`, generated by `buildSnapCommandLauncherScript` in
`app-builder-lib/dist/targets/linux/snap/snapCommand.js`), which execs the real Electron
binary — so the guard is patched into that generator via `bun patch` (see
`patches/app-builder-lib@27.0.0-alpha.7.patch`, wired through `patchedDependencies` in
`package.json`; re-apply/edit with `bun patch app-builder-lib@<version>` then
`bun patch --commit node_modules/app-builder-lib`). The patch adds a `preamble` (already
supported by the shared `buildLauncherScript` helper, just unused by the snap path before
this patch) that runs `snapctl is-connected browser-support` before the `exec` line: if
connected, `snapctl set-health okay`; if not, it prints the exact `snap connect` command
to stderr, calls `snapctl set-health blocked "Run: …"`, and exits 1 — the former covers a
terminal launch, the latter is what a GNOME/desktop launch (no attached terminal) has to
go on: `snapd-desktop-integration`, when present, turns a `set-health blocked` into a
desktop notification; otherwise it's visible via `snap warnings` or
`snap info figma-linux-next`. Verified end-to-end (both branches) against real `snapd` in
a throwaway test snap — not a substitute for connecting the interface, but replaces
Chromium's raw "Permission denied (13)" crash with a clean, actionable message and no
core dump. If `app-builder-lib` is ever bumped, re-verify `buildSnapCommandLauncherScript`
still forwards a `preamble` option before assuming the patch still applies cleanly.
...
| `patches/app-builder-lib@*.patch` | `bun patch` diff — snap launcher `browser-support` preflight (see Runtime confinement) |
